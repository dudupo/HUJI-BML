...
class BayesianLinearRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> 'BayesianLinearRegression':
        ...
        # self.theta_cov[-1] += 0.01
        # print(self.theta_cov)
        ...
        try :
            ...
            self.stablecalc = np.linalg.pinv(self.theta_cov) + design.T @ design/(self.sig**2)
            self.cov = np.linalg.pinv(self.stablecalc)
            self.mu = self.cov @ (  np.linalg.pinv(self.theta_cov) @ self.theta_mean + design.T @ y/(self.sig**2) )
    ...
    def predict_std(self, X: np.ndarray) -> np.ndarray:
        ...
        std = [ ]
        for x in X:
            std += [np.sqrt( np.diag( conj(self.basis_functions( np.array([x]) ), self.theta_cov )) + self.sig**2)[0]]
        # return np.sqrt( np.diag( conj(self.basis_functions(X), self.cov )) + np.ones(len(X))*(self.sig**2)) 
        # print("std: ")
        # print(std)
        return std
    def posterior_sample(self, X: np.ndarray) -> np.ndarray:
        ...
        sample_theta = np.random.multivariate_normal(self.theta_mean, self.theta_cov)

        return self.basis_functions(X) @ sample_theta
...
def plot_func(data, train, model, _filename):
    ...
    if isinstance(model, BayesianLinearRegression):
        std = model.predict_std(inp)
        out = model.predict(inp)
        plt.fill_between(inp, out - std, out + std, alpha=.5, label='confidence interval')
        for _ in range(5):
            plt.plot(inp, model.posterior_sample(inp))
    plt.ylim([7,30])
...
def main():
    ...
    def plot_hours_bay(pbf, mu, blr, _str):
        ...
        # print(temps[0])
        blr.fit(np.repeat(hours[:4], len(temps[:,:4])), temps[:,:4].flatten())
        plot_func( hours, temps[0] , blr, f"{_str}-baylinear-req-s" )
        
        print(hours)
        out = blr.predict( hours[4:]) 
        outw = ln.predict(hours[4:])
        # print(out)
        mmsew = float(0)
        
        mmse = float(0)
        for t in temps:
            mmse += np.sqrt((out  - t[4:]).T @ (out  - t[4:]))
            mmsew += np.sqrt((outw  - t[4:]).T @ (outw  - t[4:]))
            # print(mmse)
        
        print("mmse: {0}".format( mmse / len(temps) ))
        print("mmsew: {0}".format( mmsew / len(temps) ))
